...
function [class_membership_probabilities, class_recognition_accuracy,confmat] = predict_mRVM(TRAIN_OBJECT,Xtest,test_labels)
...
if exist('test_labels','var')
    [class_recognition_accuracy,confmat] = find_accuracy(class_membership_probabilities,test_labels');
else
...
function [y , confmat]= find_accuracy(results,t)

elements = size(results,1);
maxtable = reshape(results(:)==repmat(max(results')',size(results,2),1),size(results));
for i = 1 :110
    if sum(maxtable(i,:))==6
        maxtable(i,[1,2,3,4,6])=0;
    end
end
yper = zeros(1,110);
ytest = zeros(1,110);
for i = 1 :110
    [~,yper(i)] = find(maxtable(i,:)==1);
end
for i = 1 :110
    [~,ytest(i)] = find(t(i,:)==1);
end
confmat = confusionmat(ytest,yper);
tmp = maxtable + t;
